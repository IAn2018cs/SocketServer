...
public class ClientManager {
	...
	private static class ServerThread implements Runnable {
		...
		/**
		 * 1.获得远程服务器的IP 地址. InetAddress inetAddress = socket.getInetAddress();
		 * 2.获得远程服务器的端口. int port = socket.getPort(); 3. 获得客户本地的IP 地址.
		 * InetAddress localAddress = socket.getLocalAddress(); 4.获得客户本地的端口. int
		 * localPort = socket.getLocalPort(); 5.获取本地的地址和端口号 SocketAddress
		 * localSocketAddress = socket.getLocalSocketAddress(); 6.获得远程的地址和端口号
		...
		public void run() {
			try {
				while (!isExit) {
					...
					System.out.println("获取的手机IP地址及端口号："
